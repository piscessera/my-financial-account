...
| 34 | Create a general (non-tax) transaction | Given an open tax year, When the user toggles a transaction to "general" and picks a category (e.g. Food) instead of an income section, Then it is stored with `tax_relevant=false` and that category, with no income-section/WHT required. | Unit | AC-13 | | |
...
| 37 | Add a new deduction category (each cap shape) | Given the Settings screen, When the user creates a new category as fixed, then again as per-count, then again as shared-group-member, Then all three are stored correctly and immediately available on the Deductions screen for entry. | Unit | AC-16 | | |
