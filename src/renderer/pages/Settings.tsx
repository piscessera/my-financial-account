...
/**
 * Settings screen — Supports dedicated sub-navigation for Tax Brackets, Deduction Caps, and Storage.
 */
export default function Settings(): JSX.Element {
  const [activeSection, setActiveSection] = useState<SettingsSection>('brackets');
  ...
  return (
    <div className="page">
      <div className="page-head">
        <h1>ตั้งค่าระบบ</h1>
        <p>
          ปรับแต่งอัตราภาษีขั้นบันได เพดานค่าลดหย่อน และจัดการตำแหน่งที่เก็บฐานข้อมูล
        </p>
      </div>
      ...
      {/* Main Settings Navigation Sub-tabs (matching Entry.tsx chip-row tab style) */}
      <div className="chip-row" style={{ marginBottom: 20 }}>
        <button
          type="button"
          className={`chip${activeSection === 'brackets' ? ' active' : ''}`}
        ...
        <button
          type="button"
          className={`chip${activeSection === 'deductions' ? ' active' : ''}`}
        ...
        <button
          type="button"
          className={`chip${activeSection === 'storage' ? ' active' : ''}`}
      ...
      {/* Scope Selector Tabs for Brackets & Deductions */}
      {(activeSection === 'brackets' || activeSection === 'deductions') && (
        <>
          <div
            style={{
              marginBottom: 16,
          ...
          >
            <div className="chip-row">
              ...
              >
                📅 การตั้งค่าเฉพาะปีภาษี (Year-Specific)
              </button>
            </div>
          ...
          {activeTab === 'baseline' && (
            ...
            >
              💡 <strong>แม่แบบค่าเริ่มต้นระบบ (Baseline Template):</strong> ค่าในส่วนนี้จะถูกนำไปใช้เป็นค่าตั้งต้นอัตโนมัติเมื่อสร้างปีภาษีใหม่
            </div>
          )}
          ...
          {isYearClosed && (
            ...
            >
              🔒 <strong>ปีภาษีนี้ถูกปิดรอบแล้ว (Closed Tax Year):</strong> ตามกฎความถูกต้องทางบัญชี (INV-7) ข้อมูลของปีที่ปิดแล้วจะไม่สามารถแก้ไขได้
            </div>
          )}
        </>
      )}
            ...
            <table className="bracket-table">
              <thead>
                <tr>
                  ...
                  <th className="center">อัตราภาษี</th>
              ...
              <tbody>
                {brackets.map((bracket) => (
                  <tr key={bracket.id}>
                    <td style={{ fontWeight: 500 }}>{formatBracketRange(bracket)}</td>
                    <td className="center">
                      <span
                        className="tag"
                        style={{
                          background: bracket.rateBp === 0 ? 'var(--good-soft)' : 'var(--accent-soft)',
                          color: bracket.rateBp === 0 ? 'var(--good)' : 'var(--accent)',
                          fontWeight: 600,
                        }}
                      >
                        {bracket.rateBp === 0 ? 'ยกเว้นภาษี (0%)' : `${(bracket.rateBp / 100).toFixed(0)}%`}
                      </span>
                    </td>
                    <td className="right">
                      {!isYearClosed && (
                ...
                <tr key={category.id}>
                  ...
                  <td className="center">
                    <span className={`pill ${category.isActive ? 'active' : 'voided'}`}>
                      {category.isActive ? 'ใช้งานอยู่' : 'ปิดใช้'}
                    </span>
                  </td>
                  <td className="right">
                    {!isYearClosed && (
            ...
            <div className="panel" style={{ marginTop: 20, border: '1px solid var(--accent)' }}>
              ...
              <div className="form-grid">
                <div className="field span2">
                  ...
                  <input
                    type="text"
                    placeholder="เช่น ค่าลดหย่อนโครงการพิเศษ"
      ...
      {activeSection === 'storage' && (
        <div className="panel">
          ...
          >
            <span>📁 ตำแหน่งที่จัดเก็บไฟล์ฐานข้อมูล (Database Storage)</span>
            <button
              type="button"
              className="btn btn-ghost"
          ...
          {dataLocation ? (
            <>
              <div className="field span2" style={{ marginTop: 12 }}>
                <label>โฟลเดอร์ฐานข้อมูลปัจจุบัน</label>
                <div className="control num" style={{ fontSize: 13, padding: '10px 14px' }}>
              ...
              <div className="muted" style={{ marginTop: 6, lineHeight: 1.6 }}>
                💡 <strong>คำแนะนำ:</strong> สามารถเลือกโฟลเดอร์ที่ซิงค์กับ Google Drive Desktop
                เพื่อให้ข้อมูลสำรองอัตโนมัติบน Cloud โดยที่โปรแกรมอ่านเขียนไฟล์แบบ Offline โดยตรง
              </div>
            </>
          ) : (
          ...
          {folderChange?.step === 'confirmMove' && (
            <div className="panel" style={{ marginTop: 20, border: '1px solid var(--amber)' }}>
              <div className="section-label">ย้ายข้อมูลไปโฟลเดอร์นี้?</div>
          ...
          {folderChange?.step === 'warnExisting' && (
            <div className="panel" style={{ marginTop: 20, border: '1px solid var(--amber)' }}>
              <div className="section-label">พบไฟล์ข้อมูลอยู่แล้วในโฟลเดอร์นี้</div>
              <p className="control num" style={{ fontSize: 13 }}>
                {folderChange.targetFolderPath}
              </p>
              <p className="muted">
